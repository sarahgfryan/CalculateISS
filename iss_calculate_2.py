...
try:
    ...
    baseline_data = baseline_data[['PUBLIC_ID', 'D_LAB_serum_beta2_microglobulin','D_LAB_chem_albumin','D_LAB_chem_ldh']]
    # Rename the columns
    ...
    # Filter the data to get LDH, B2M and alb from the 3 month visit data
    three_month_data = patient_visit_data[patient_visit_data["VJ_INTERVAL"] == "Month 3"]
    # Select the columns of interest
    three_month_data = three_month_data[['PUBLIC_ID', 'D_LAB_serum_beta2_microglobulin','D_LAB_chem_albumin','D_LAB_chem_ldh']]
    # Rename the columns
    three_month_data.columns = ['Patient_ID', 'b2m_3', 'alb_3', 'ldh_3']

    # Drop duplicates for 3 month visits
    three_month_data = three_month_data.drop_duplicates(subset='Patient_ID', keep='first')

    # Merge the 3-month LDH data into baseline data
    baseline_data = pd.merge(baseline_data, three_month_data, on='Patient_ID', how='left')
    # Update the columns in baseline_data where it is NaN
    baseline_data['b2m'] = baseline_data['b2m'].fillna(baseline_data['b2m_3'])
    baseline_data['alb'] = baseline_data['alb'].fillna(baseline_data['alb_3'])
    baseline_data['ldh'] = baseline_data['ldh'].fillna(baseline_data['ldh_3'])
    # Drop the extra columns after merging
    del baseline_data['b2m_3']
    del baseline_data['alb_3']
    del baseline_data['ldh_3']
    ...
    # Add the underscore in Patient_ID to match the baseline data
    iss_data['Patient_ID'] = iss_data['Patient_ID'].apply(lambda x: re.sub(r'(MMRF)(\d+)', r'\1_\2', x))
    ...
    # Check for discrepancies. Filter out rows where both ISS_calc and ISS_Compass are NaN
    ...
    merged_data.to_excel("ISS_2.xlsx", index=False)
    print("\nData saved successfully to ISS_2.xlsx")
